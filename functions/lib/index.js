...
const DEFAULT_GAME = functions.config().faceit?.game || 'cs2';
function getApiKey() {
    const key = process.env.FACEIT_API_KEY || functions.config().faceit?.apikey;
    if (!key)
        throw new Error('Missing functions config: faceit.apikey');
    return key;
}
...
function getEnv(name, required = true) {
    ...
    if (!v && required)
        console.warn(`[OAUTH] Missing env ${name}`);
    return v;
}
...
app.get('/auth/faceit/callback', async (req, res) => {
    try {
        ...
        try {
            ...
            console.warn('[OAUTH] Failed to persist token to Firestore:', e?.message || e);
        }
...
app.get('/faceit/search', async (req, res) => {
    try {
        ...
        const enriched = await Promise.all(data.items.map(async (p) => {
            try {
...
app.get('/faceit/players/:id/matches/details', async (req, res) => {
    try {
        ...
        const hist = await faceitFetch(`/players/${id}/history`, { game, limit, offset });
        ...
        for (const [chunkIndex, group] of chunks.entries()) {
            ...
            for (const md of results) {
                if (!md)
                    continue;
                const allPlayers = [];
                const teamByKey = { faction1: md?.teams?.faction1, faction2: md?.teams?.faction2 };
                ...
                if (statsData?.rounds?.length > 0) {
                    ...
                    try {
                        ...
                        const teamsArr = Array.isArray(firstRound?.teams) ? firstRound.teams : [];
                        const scoreByFaction = { faction1: 0, faction2: 0 };
                        for (const team of teamsArr) {
                            const factionId = team?.team_id;
                            if (factionId !== 'faction1' && factionId !== 'faction2')
                                continue;
                            ...
                            if (parsed > 0)
                                scoreByFaction[factionId] = parsed;
                        }
                        ...
                        console.log(`[MATCHES] Error while reading team_stats scores from stats`, e?.message || e);
                    }
                    ...
                    if (team1Score === 0 && team2Score === 0) {
                        try {
                            ...
                            console.log(`[MATCHES] Error while parsing round_stats.Score`, e?.message || e);
                        }
                    }
                    ...
                    if (team1Score === 0 && team2Score === 0 && statsData.rounds.length > 1) {
                        ...
                        for (const round of statsData.rounds) {
                            if (round?.winner === 'faction1')
                                faction1Rounds++;
                            else if (round?.winner === 'faction2')
                                faction2Rounds++;
                        }
                    ...
                    for (const round of md.stats.rounds) {
                        ...
                        for (const team of teams) {
                            ...
                            if (playerStats?.player_stats) {
                                kills = num(playerStats.player_stats?.Kills || playerStats.player_stats?.kills);
                                deaths = num(playerStats.player_stats?.Deaths || playerStats.player_stats?.deaths);
                                headshots = num(playerStats.player_stats?.Headshots || playerStats.player_stats?.headshots);
                ...
                if (kills === 0 && deaths === 0 && me?.player_stats) {
                    ...
                    headshots = num(me.player_stats?.Headshots || me.player_stats?.headshots);
            ...
            for (const md of results) {
                ...
                for (const p of allPlayers) {
                    if (p.player_id === id)
                        continue;
                    if (p.team !== myTeam)
                        continue; // only teammates, not opponents
...
app.get('/faceit/debug/match-quick/:matchId', async (req, res) => {
    try {
        ...
        const quickAnalysis = {
            ...
            possibleRoundScores: {
                fromDetailedResults: md?.detailed_results?.map((r) => r?.factions) || [],
